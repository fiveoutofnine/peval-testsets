    ...
    try:
        ...
        with open(csv_path, 'r') as f:
            ...
            for row in reader:
                ...
                stats["by_elo_phase_type"][row["elo_bucket"]][row["phase"]][row["type"]] += 1
...
def main():
    ...
    if db_stats:
        ...
        for elo_range, count in db_stats["elo_distribution"]:
            percentage = (count / db_stats["total_games"] * 100) if db_stats["total_games"] > 0 else 0
    ...
    if pos_stats:
        ...
        for phase in ["opening", "middlegame", "endgame"]:
            ...
            percentage = (count / pos_stats["total"] * 100) if pos_stats["total"] > 0 else 0
            target = phase_targets.get(phase, 0)
            rows.append([phase.capitalize(), str(count), f"{percentage:.1f}%", f"{target}%"])
        print(format_table(headers, rows))
    ...
    if q_stats:
        ...
        rows = [
            ["Public", str(q_stats["public"]), f"{q_stats['public'] / q_stats['total'] * 100:.1f}%"],
            ["Private", str(q_stats["private"]), f"{q_stats['private'] / q_stats['total'] * 100:.1f}%"],
