...
def evaluate_all_moves(board: chess.Board, engine: chess.engine.SimpleEngine) -> List[Tuple[str, int]]:
    """
            ...
            for move in remaining_moves:
                ...
                try:
                    ...
                    result = engine.analyse(
                        board,
                        chess.engine.Limit(depth=reduced_depth, time=0.1)
                    )
                    if "score" in result:
                        score = result["score"]
                        if score.is_mate():
...
def main():
    ...
    print(f"Engine configuration: depth={ENGINE_CONFIG['depth']}, time={ENGINE_CONFIG['time_limit']}s")
    ...
    with open(temp_file, 'w', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=['index', 'input', 'expected_output', 'private'])
        writer.writeheader()
        ...
        for i, pos in enumerate(positions):
            ...
            try:
                ...
                print(f"  Evaluating all moves...", end='', flush=True)
                ...
                if move_scores:
                    ...
                    score_str = f"{best_score/100:.2f}" if abs(best_score) < 20000 else "Mate"
