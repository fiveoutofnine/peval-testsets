...
def run_step(step_name: str, script: str, check_files: list = None, force: bool = False) -> bool:
    """
    Run a pipeline step.
...
def main():
    ...
    if not run_step("Step 2: Process games", "02_process.py", 
                   ["output/games.db"], force=force_all):
        print("\n✗ Pipeline failed at Step 2")
        sys.exit(1)
    
    # Step 3: Select positions
    if not run_step("Step 3: Select positions", "03_select.py", 
                   ["output/positions.csv"], force=force_all):
    ...
    if os.path.exists("output/positions.csv"):
        ...
        if position_count < 800:
            ...
            print(f"  Missing {800 - position_count} positions (likely due to limited 2600+ ELO games)")
